...
#SBATCH --array=1                                      # this is an array job, where you launch one job per array item (and you can pass different parameters to each job, see below)
#SBATCH --ntasks 1 
...
python3 /home/prat/slurm-main.py star $SLURM_ARRAY_TASK_ID $N_graph $M_graph $log_s_min $log_s_max $nb_trajectories $migration_rate $nb_demes $alpha 0 ## starts on the center

python3 /home/prat/slurm-main.py star $SLURM_ARRAY_TASK_ID $N_graph $M_graph $log_s_min $log_s_max $nb_trajectories $migration_rate $nb_demes $alpha 1 ## starts on the leaf
